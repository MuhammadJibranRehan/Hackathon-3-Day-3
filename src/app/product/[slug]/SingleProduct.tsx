...
const SingleProduct: FC<IProducts> = ({
  title,
  price,
  image,
  _id,
  ...
  const handleImageClick = (img: IImage) => setSelectedImage(img);
    ...
    <div>
      <div className="md:flex lg:mx-16 mx-4 items-center mt-10">
        <div className="flex gap-3 flex-row md:block">
          {image.map((img) => (
            <div key={img._id} onClick={() => handleImageClick(img)}>
              <Image
                src={urlForImage(img).url()}
                alt={title}
                width={140}
                height={140}
                className="m-1 my-3 rounded-sm bg-gray-100"
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col ml-12 lg:ml-24">
          <h1 className="font-bold font-sans text-3xl text-[#1A0B5B]">
            {title}
          </h1>
          <p className="uppercase text-gray-400 my-2 font-bold">
            {category.name}
          </p>
      ...
      <div className="mx-10 mt-24 md:mt-10">
        <div className="mt-10 space-y-2">
          <h2 className="text-2xl font-bold font-serif text-[#1A0B5B]">
            Product Description:
          </h2>
          <p className="text-justify font-sans font-semibold text-[#8A8FB9]">
            {description}
          </p>
        </div>
        <div className="mt-10 space-y-2">
