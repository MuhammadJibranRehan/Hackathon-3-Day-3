...
import StripeCheckout from '@/app/cart/StripeCheckout';
import DeleteSingleCart from './DeleteSingleCart';

export const getData = async () => {
  try {
    const LOCAL_URL = 'http://localhost:3000/api/cart';
    const DEPLOYED_URL = 'https://hackathon-3-day-4.vercel.app/api/cart';
    ...
    const cartData: CartTypes[] = await res.json();
...
const Cart = async () => {
  ...
  const uid = res.data.filter((item) => item.user_id === user_id);
      ...
      <div className="flex flex-col lg:flex-row justify-around mt-10">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-[#E7E4F8] ">
                <TableHead className="w-fit px-8">
                  <h4 className="font-bold text-[#30207a] text-lg">
                    Product Name
                  </h4>
                </TableHead>
                <TableHead className="text-center px-8">
                  <h4 className="font-bold text-[#30207a] text-lg">Quantity</h4>
                </TableHead>
                <TableHead className="text-center px-8">
                  <h4 className="font-bold text-[#30207a] text-lg">Price</h4>
                </TableHead>
                <TableHead className="text-center px-8">
                  <h4 className="font-bold text-[#30207a] text-lg">
                    Total Price
                ...
                <TableHead className="text-center px-8">
                  <h4 className="font-bold text-[#30207a] text-lg">Delete</h4>
                </TableHead>
              </TableRow>
            </TableHeader>
            {uid.length > 0 ? (
              uid.map((item: CartTypes) => {
                return (
                  <TableBody className="bg-[#F4F4FC]" key={item.id}>
                    <TableRow>
                      <TableCell className="flex items-center w-fit">
                        ...
                        />
                        <div>
                          <h4 className="font-bold text-[#30207a]">
                            {item.name}
                          </h4>
              ...
              <p className="flex items-center justify-center w-fit font-extrabold text-2xl">
                Your Cart is Empty!
              </p>
            )}
          </Table>
        ...
        <div className="m-4 lg:m-0 lg:mt-0 mt-8 md:mx-10">
          <Table>
            <TableHeader>
              <TableRow className="bg-[#E7E4F8] ">
                <TableHead className="w-fit px-12">
                  <h4 className="font-bold text-[#30207a] text-center text-lg">
                    Cart Total
                  </h4>
                </TableHead>
              </TableRow>
          ...
          <div className="bg-[#F4F4FC] rounded-sm p-10">
            {uid.map((item: CartTypes) => {
              ...
              const itemPrice = parseInt(item.total);
